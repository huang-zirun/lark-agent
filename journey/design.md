...
Last updated: 2026-05-06 (Dashboard stage detail completion)
...
- In scope: unified human-readable artifact publication to Feishu cloud docs for requirement intake, solution design, code generation, test generation, code review, and delivery. Publication results are recorded under `run.json.artifact_publications`; machine-readable JSON/diff artifacts remain local in the run directory.
...
- Stage artifact documents use `lark.artifact_folder_token` as the preferred target folder, falling back to the legacy `lark.prd_folder_token` when unset.
- Cloud document creation uses `lark-cli docs +create --as bot --title ... --markdown ... --folder-token ...`; the old create path using `--doc-format/--content/--parent-token` is no longer used for publication.
...
- **Event format normalization**: `event_to_source()` in `intake/lark_cli.py` handles both flat event formats (legacy) and nested SDK formats (real `im.message.receive_v1`). The `sender_id` field is extracted via `_resolve_sender_id()` which traverses `sender.sender_id.{open_id,user_id,union_id}` for nested structures. This ensures consistent user identification across `SessionManager`, `find_blocked_workspace_run()`, and `_route_active_session()` regardless of event format.
...
- The **second checkpoint** (code review) now supports the same **dual-channel architecture** as the first checkpoint:
  1. **Lark External Approval (primary)** — when `approval.enabled` is true and a sender_id is available, `publish_code_review_checkpoint()` creates a separate code-review-specific external approval definition (`devflow-code-review`) and instance via `ensure_stage_approval_definition(stage="code_review")` and `create_stage_approval_instance(stage="code_review")`. The approval title is "代码评审 - {run_id}" with stage-specific i18n labels (评审摘要, 评审报告, 阻塞问题数). The user receives the approval todo in the Feishu Approval app with the same "Approve"/"Reject" buttons; rejection requires a reason.
  2. **Bot message replies + card (fallback)** — when approval is disabled or creation fails, the system falls back to the card-based channel with `Approve/Reject <run_id>` text commands. The card includes a hint "也可在飞书「审批」应用中处理" when an approval instance exists.
  3. **State synchronization** — when a code review checkpoint is approved or rejected via IM text command, `_sync_approval_instance_if_needed()` updates the external approval instance status to keep both channels consistent. This mirrors the solution review's existing sync behavior via `update_external_approval_instance()`.
  4. **Stage-aware approval functions** — `approval_client.py` now provides `ensure_stage_approval_definition(stage)` and `create_stage_approval_instance(stage)` with a `_STAGE_APPROVAL_CONFIG` dict that maps stage names to their approval configuration (default_code, title, instance_title, task_title, doc_label, summary_label). The original `ensure_approval_definition()` and `create_external_approval_instance()` are preserved as backward-compatible wrappers delegating to `stage="solution_design"`.
...
- LLM calls expose a rich completion result internally so the pipeline can persist raw request/response payloads, provider token usage, usage source, request duration, and `finish_reason` while keeping API keys and Authorization headers out of audit files.
- **Reasoning model support**: Reasoning models (e.g., MiMo v2.5-pro) consume `reasoning_tokens` that count toward `completion_tokens`. To prevent truncation, `max_tokens` defaults to 4096 (up from 2000). When `finish_reason="length"` is detected, the system throws a clear error suggesting to increase `max_tokens` or shorten the prompt. Truncated JSON responses are automatically repaired by closing unterminated strings and brackets.
...
- Dashboard stage detail enhancement: (1) Each stage card now displays LLM info inline — model name, provider, token consumption, LLM call duration — sourced from per-stage `token_summary` and `llm_calls` records. (2) `get_run_llm_trace` normalizes intake `llm-request/response.json`, solution `solution-llm-request/response.json`, and multi-turn `code/test/review-llm-request/response-turnN.json` files into canonical stage names with full prompts, output, token usage, timing, provider, model, turn, and audit paths. (3) `GET /api/v1/metrics/runs/{run_id}/artifact-markdown?stage={stage}` serves all six built-in stage Markdown files, preferring `run.json` path fields such as `requirement_markdown` and falling back to conventional filenames. (4) Frontend `ArtifactCard` renders Markdown content inline with a lightweight `simpleMarkdownToHtml` converter (supports headings, bold/italic, inline code, lists, code blocks, tables), default-collapsed with 10-line preview. (5) LLM trace panel filters by selected stage. (6) `useRunLlmTrace` auto-polls every 3s for active runs; `useRunDetail` active polling shortened to 2s; `useRunList` polling shortened to 5s.
- Dashboard stage detail Tab view: The right panel is restructured from a fixed `LlmTracePanel` into a `StageDetailView` component with two tabs — "阶段产物" (default) and "LLM 推理". Clicking a stage in `PipelineStepper` switches the right panel to that stage's detail view and resets the right panel to the default "阶段产物" tab. The "阶段产物" tab uses `useRunArtifactMarkdown` to fetch and render the full Markdown document for the selected stage (PRD for requirement_intake, tech spec for solution_design, code generation report, test generation report, review report for code_review, delivery notes for delivery). The "LLM 推理" tab reuses `LlmCallItem` from `LlmTracePanel` to show filtered LLM call records. The left `StageDetailPanel` is simplified to show only stage status overview cards (name, status, duration, model, tokens), with artifact/diff/approval rendering removed. `simpleMarkdownToHtml` is extracted to `lib/markdown.ts` as a shared utility. The `scrollIntoView` behavior on stage click is removed.
- **Session-aware context association** (`devflow.session`): The pipeline now maintains an in-memory session state keyed by `chat_id:sender_id` to track active runs per user. This solves the "context fragmentation" bug where follow-up messages sent after the 5-second merge window expired were incorrectly treated as new requirements. When a user sends a message and an active run exists in the same session, the message is routed to that run instead of creating a new one. The `SessionManager` provides `register()`, `unregister()`, `lookup()`, and `update_status()` with automatic timeout cleanup (default 1800s). Message routing priority now includes an "active session context routing" layer: `waiting_clarification` → clarification reply, `blocked` → workspace recovery or append, `running` (requirement_intake) → append to requirement, `waiting_approval` → approval prompt, `running` (other stages) → queue with "processing" message. The `find_*_run` functions (`find_waiting_clarification_run`, `find_blocked_workspace_run`, `find_awaiting_reject_reason_run`) now check `SessionManager` first before falling back to filesystem scan. `MessageBuffer` treats first-line workspace directives such as `仓库：...` and `新项目：snake-game` as control events, so they flush any buffered requirement instead of being merged into it. Configurable via `interaction.session_timeout_seconds` in `config.json`.
...
Pipeline run records use `schema_version = "devflow.pipeline_run.v1"` and include trigger metadata, detected input, stage statuses, start/end timestamps, error details, artifact paths, publication status for PRD document/card delivery, `artifact_publications` keyed by stage, `pipeline_config`, `graph_state`, `lifecycle_status`, and optional `provider_override`.
